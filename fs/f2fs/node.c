				...
				enum iostat_type io_type, unsigned int *seq_id)
{
	...
	if (__is_valid_data_blkaddr(ni.blk_addr) &&
		!f2fs_is_valid_blkaddr(sbi, ni.blk_addr,
					DATA_GENERIC_ENHANCE)) {// 如果ni.blk_addr指向的数据块有效，会报错
		up_read(&sbi->node_write);
		goto redirty_out;
	}
...
start_flush:
	if (to_journal) {
		...
		p = (struct node_info *)(PM_I(sbi)->p_nat_va_start);
	...
	list_for_each_entry_safe(ne, cur, &set->entry_list, list) {
		...
		if (to_journal) {	// 如果写journal，获得中raw_ne的地址
			...
			f2fs_write_nat_to_pm(&ne->ni, (void *)(&p[nid]));
...
skip_copy:
	...
	if (to_journal) {
		...
		__update_nat_bits_based_on_pm(sbi, start_nid);
	} else {
